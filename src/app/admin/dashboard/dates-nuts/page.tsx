...
import { Plus, Trash2, Edit, X, Image as ImageIcon, Nut, FolderPlus } from 'lucide-react';
...
export default function DatesNutsAdmin() {
  ...
  const handleCategorySubmit = async (e: React.FormEvent) => {
    ...
    if (categoryFormData.id) {
      ...
      // Add
      const { error } = await supabase.from('product_categories').insert([{ title: categoryFormData.title, section: 'dates_nuts' }]);
      if (!error) {
        setIsCategoryModalOpen(false);
        setCategoryFormData({ id: '', title: '' });
        fetchProductsAndCategories();
            ...
            <div className="p-6 overflow-y-auto flex-1">
              <form id="productForm" onSubmit={handleSubmit} className="grid grid-cols-2 gap-6">
                ...
                <div className="col-span-2">
                  <label className="block text-[#5C3D2E] text-xs font-bold uppercase tracking-wider mb-2">Product Image *</label>
                  <div className="flex items-center gap-4">
                    {formData.image ? (
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col">
            ...
            <div className="p-6">
              <form id="categoryForm" onSubmit={handleCategorySubmit}>
                ...
                <input type="text" value={categoryFormData.title} onChange={e => setCategoryFormData({...categoryFormData, title: e.target.value})} className="w-full px-3 py-2 bg-[#F8F2EA] border border-[#DCD0C3] rounded focus:outline-none focus:border-[#8B3A2B]" required placeholder="e.g. Premium Dates" />
              </form>
            </div>
            <div className="px-6 py-4 border-t border-[#DCD0C3] bg-[#F8F2EA] flex justify-end gap-3">
