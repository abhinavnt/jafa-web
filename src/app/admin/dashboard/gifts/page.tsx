...
export default function GiftsAdmin() {
  ...
  const handleCategorySubmit = async (e: React.FormEvent) => {
    ...
    if (categoryFormData.id) {
      ...
      // Add
      const { error } = await supabase.from('product_categories').insert([{ title: categoryFormData.title, icon: categoryFormData.icon, section: 'gifts' }]);
      if (!error) {
        setIsCategoryModalOpen(false);
        setCategoryFormData({ id: '', title: '', icon: '' });
        fetchProductsAndCategories();
    ...
    <div className="max-w-6xl mx-auto">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-[#DCD0C3]">
              {loading ? (
                <tr>
                  <td colSpan={3} className="p-8 text-center text-[#5C3D2E] sm:hidden">Loading gifts...</td>
                  <td colSpan={5} className="p-8 text-center text-[#5C3D2E] hidden sm:table-cell">Loading gifts...</td>
                </tr>
              ) : products.length === 0 ? (
                <tr>
                  <td colSpan={3} className="p-8 text-center text-[#5C3D2E] sm:hidden">No gifts found.</td>
                  <td colSpan={5} className="p-8 text-center text-[#5C3D2E] hidden sm:table-cell">No gifts found.</td>
                </tr>
              ) : (
                products.map((p) => (
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden max-h-[90vh] flex flex-col">
            <div className="px-6 py-4 border-b border-[#DCD0C3] flex justify-between items-center bg-[#F8F2EA]">
              <h3 className="font-bold text-[#2A1A12] text-lg">{editingId ? 'Edit Gift' : 'Add New Gift'}</h3>
              ...
              <form id="productForm" onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="col-span-2 border border-[#DCD0C3] p-4 rounded bg-[#F8F2EA]/50">
                  <div className="flex items-center justify-between mb-3">
                    <label className="block text-[#5C3D2E] text-xs font-bold uppercase tracking-wider">Product Variants (Sizes/Weights/Types)</label>
                    <button 
                      type="button" 
                    ...
                    <div className="space-y-3">
                      {formData.variants.map((v, i) => (
                        <div key={i} className="flex flex-wrap sm:flex-nowrap gap-3 items-start">
                          <input 
                            type="text" 
                            placeholder="Variant Name (e.g. Medium Box)" 
                            value={v.name}
                            onChange={(e) => {
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col">
            ...
            <div className="p-6">
              <form id="categoryForm" onSubmit={handleCategorySubmit}>
                ...
                <input type="text" value={categoryFormData.title} onChange={e => setCategoryFormData({...categoryFormData, title: e.target.value})} className="w-full px-3 py-2 bg-[#F8F2EA] border border-[#DCD0C3] rounded focus:outline-none focus:border-[#8B3A2B] mb-4" required placeholder="e.g. Gift Hampers" />
