...
export default function Categories({ categories }: CategoriesProps) {
  return (
    <section className="w-full max-w-7xl mx-auto px-4 lg:px-6 pt-8 pb-16 md:pt-12 md:pb-24 lg:pt-16">
      ...
      <div className="hidden lg:grid grid-cols-3 gap-6">
        {categories.map((category) => {
          ...
          return (
            <div key={category.id} className="group cursor-pointer bg-[#F2EAE0] border border-[#E2D2C2] rounded-xl overflow-hidden flex flex-row items-stretch transition-all hover:shadow-md hover:border-[#D4C3B3]">
              {/* Image Side */}
      ...
      <div className="flex lg:hidden flex-row justify-center items-start gap-4 sm:gap-12">
        {categories.map((category) => (
          <div key={category.id} className="flex flex-col items-center group cursor-pointer flex-1 max-w-[120px] sm:max-w-[150px]">
